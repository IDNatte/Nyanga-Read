<script lang="ts">
  ...
  onMount(() => {
    accordHeaderClassEvent = (event: Event) => {
      if (get(accordionStore).accordionOpen === null || get(accordionStore).accordionOpen === 'close' && get(accordionStore).accordionId === null) {
        accordionStore.set({accordionOpen: 'open', accordionId: accordClass.classList[0]})
      } else if (get(accordionStore).accordionOpen === 'open' && get(accordionStore).accordionId === accordClass.classList[0]) {
...
<section class="{`accordion-${(Math.random() + 1).toString(36).substring(7)}`}" bind:this={accordClass}>
  <header>
